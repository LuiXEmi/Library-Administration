...
namespace Interfaz
{
    public partial class frmBuscarRenta : Form
    {
        ...
        private void frmBuscarRenta_Load(object sender, EventArgs e)
        {
        ...
        private void bttnEntrega_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("ERROR: Debes introducir un Id de Renta existente");
                txtBuscarId.Focus();
            }
            if(lblEstado.Text == "ENTREGADO")
            {
                MessageBox.Show("LA RENTA YA HA SIDO ENTREGADA");
                txtBuscarId.Focus();
            }
            else
            {
                try
                {
                    ...
                    NpgsqlCommand cmdUp = new NpgsqlCommand
                        ("update renta set estado='ENTREGADO' where id_renta=" + BUSCADO + "", dbcon);
                    if (MessageBox.Show("Deseas Confirmar la Entrega?:" ,"CONFIRMA", MessageBoxButtons.YesNo,MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)== DialogResult.Yes)
                    {
                        ...
                        MessageBox.Show("RENTA ENTREGADA");
                    }
                    txtBuscarId.Clear();
                    txtIdEmpleado.Clear();
        ...
        private void bttnBuscar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    dbcmd.CommandText = "select * from renta where id_renta=" + txtBuscarId.Text + "";
                    ...
                    if (reader.Read())
                    {
                        ...
                        lblFecha.Text = reader.GetString(reader.GetOrdinal("fecha_inicio"));
                        lblFechaEntrega.Text = reader.GetString(reader.GetOrdinal("fecha_entrega"));
                    ...
                    if (band == 1)
                    {
                        ...
                        comandoDetalleVenta.CommandText = "select * from renta_libro where id_renta=" + Convert.ToInt16(lblIdRenta.Text) + "";
                    ...
                    if (band == 0)
                    {
                        MessageBox.Show("Renta No Encontrada......");
                    }
